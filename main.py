...
def get_predicted_value(patient_symptoms):
    ...
    for item in patient_symptoms:
        if item in symptoms_dict:  # ✅ only accept valid symptoms
            input_vector[symptoms_dict[item]] = 1
            valid_symptoms.append(item)

    if not valid_symptoms:  # ✅ if no symptoms matched
        return None
...
def home():
    if request.method == 'POST':
        ...
        if not symptoms or symptoms.lower() == "symptoms":
            message = "⚠️ Please enter at least one valid symptom."
        ...
        if predicted_disease is None:  # ✅ no valid symptoms
            message = "⚠️ No matching symptoms found. Please check spelling or try different symptoms."
            return render_template('index.html', message=message, symptoms=list(symptoms_dict.keys()))
